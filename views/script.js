...
function search(){
    ...
    ul.innerHTML="";
    if(value){
        axios.get(`http://localhost:5000/getdate/${value}`)
        .then(result=>{
            if(result.data[0].length>0){
                ...
                result.data[0].forEach(element => {
                    const status=element.present===1?"✅️":"❌️";
                    html+=`<tr>
                    <td>${element.id}</td>
                    <td>${element.name}</td>
                    <td>${status}</td>
...
function submitData(event,records,date){
    ...
    if(check){
        axios.post(`http://localhost:5000/postattendance`,{date:date,data:arr})
        .then(result=>{
            console.log(result);
...
document.querySelector('#fetch-btn').addEventListener('click',()=>{
    axios.get("http://localhost:5000/getreport")
    .then(result=>{
